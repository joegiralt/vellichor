...
class DoubleEndedQueue {
   ...
   * @returns {*}
   */
  public peekAt(index: number): any {
    let idx = index;
   ...
   * @returns {*}
   */
  public removeOne(index: number): any {
    let idx = index;
    // expect a number or return undefined
    if (idx !== (idx | 0)) {
      return void 0;
   ...
   * @returns {array}
   */
  public remove(index: number, count?: number): any[] {
    let idx = index;
    let removed: any[];
   ...
   * @returns {array}
   */
  public splice(index: number, count?: number, ...extraArgs: any[]): any[] {
    let idx = index;
    // expect a number or return undefined
    if (idx !== (idx | 0)) {
      return void 0;
    ...
    if (extraArgs.length > 0) {
      ...
      if (!size || idx < size / 2) {
        ...
        for (k = 0; k < tempLength; k++) {
          temp[k] = this.list[
            (this.head + idx + count + k) & this.capacityMask
          ];
        }
        if (count === 0) {
          removed = [];
   ...
   * @private
   */
  private copyArray(fullCopy: boolean): any[] {
    const newArray = [];
